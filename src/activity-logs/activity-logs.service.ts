...
export class ActivityLogsService {
  ...
  async createActivityLog(username: string, method: string, url: string, status: number, description: string): Promise<IActivityResponse> {
    try {
      const actEntity = populateToActivityEntity(username, method, url, status, description);
